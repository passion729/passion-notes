...
<Callout title="Use snake case keys">
    ```rust
    // [!code highlight]
...
**Option 2**: Wrap the return type in a Result. This one is a bit harder to implement, but works for all types. The same like [Return an error handling](./command.mdx#return-an-error-handling).
```rust
